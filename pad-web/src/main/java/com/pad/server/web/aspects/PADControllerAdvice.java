package com.pad.server.web.aspects;
...
public class PADControllerAdvice {
    ...
    @ExceptionHandler({ PADOnlinePaymentException.class })
    @ResponseStatus(value = HttpStatus.BAD_REQUEST)
    public ApiResponseJsonEntity handlePADOnlinePaymentException(HttpServletRequest request, PADOnlinePaymentException padope) {

        ApiResponseJsonEntity result = new ApiResponseJsonEntity();
        result.setResponseCode(padope.getResponseCode());
        result.setResponseText(padope.getResponseText());
        result.setResponseDate(new Date());
